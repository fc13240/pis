			...
			<div class="mod1_cont" id="mod1_cont" runat="server">
				...
				<c:forEach items="${article}" var="article">
					...
					<span class="h_list_tit">
						<a href="<s:url value='/article/articlePreview.html'/>?articleId=<c:out value='${article.id}'/>" target="_blank" title="${article.title}">${article.title}</a>
					</span> 
					<span class="h_list_deta"><fmt:formatDate value="${article.publishTime}" pattern="yyyy年MM月dd日 HH:mm"/></span> 
					</li>			
				</c:forEach>
...
function gotoPage() {
	...
	var url = "<s:url value='/article/articleList.html'/>?currentPage=" + pageNo;
